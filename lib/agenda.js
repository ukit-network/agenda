...
var Agenda = module.exports = function (config) {
    ...
    DataConnector = require('./' + (config.dataConnectorName ? config.dataConnectorName : 'mongodb'));

    _self._name = config.name;

    _self._db = new DataConnector({db: config.db, mongo: config.mongo}, function(err, data){

        if(!err){
            _self.emit('initialized');
        } else {
            _self.emit('failed');
        }
    ...
    _self._processEvery = humanInterval(config.processEvery) || humanInterval('5 seconds');
    _self._defaultConcurrency = config.defaultConcurrency || 5;
    _self._maxConcurrency = config.maxConcurrency || 20;
    _self._definitions = {};
    _self._runningJobs = [];
    _self._jobQueue = [];
    _self._defaultLockLifetime = config.defaultLockLifetime || 10 * 60 * 1000; //10 minute default lockLifetime
...
Agenda.prototype.mongo = function () {
    return this;
};
...
Agenda.prototype.database = function () {
    return this;
};
...
Agenda.prototype.stop = function (cb) {
    ...
    unlockJobs.call(this, function(err, data){
        _self.emit('stopped');
        cb(err, data);
    });
...
function processJobs(extraJob) {
    ...
    function jobProcessing() {
        ...
        if(!job || !name || !jobDefinition){
            return;
            // console.log('Missing Data'+utils.inspect({job: job, name: name, jobDefinition: jobDefinition},{showHidden: true, depth: 10, colors: true}));
        ...
        function runOrRetry() {
            if (self._processInterval) {
                if (jobDefinition && jobDefinition.concurrency > jobDefinition.running &&
